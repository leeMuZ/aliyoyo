...
<div id="content">
  ...
  <div class="container-fluid">
        ...
        <div class="panel panel-default">
          ...
          <table class="table">
            ...
            <tr>
              ...
              <td><a href="mailto:<?php echo $email; ?>">785567930@qq.com</a></td>
            </tr>
            <tr>
      ...
      <div class="panel-body">
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            ...
            <tr>
              <td class="text-left"><a href="<?php echo $voucher['href']; ?>"><?php echo $voucher['name']; ?></a></td>
              <td class="text-right">1</td>
              <td class="text-right">$<?php echo $voucher['back_price']; ?></td>
              <td class="text-right">$<?php echo $voucher['total']; ?></td>
            </tr>
            <?php } ?>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-comment-o"></i> <?php echo $text_express; ?></h3>
      </div>
      <div class="panel-body">
      <div id="express"></div>
      <div class="input-group">
                      <input type="text" name="express_num" value="<?php if($express_num)echo $express_num; ?>" id="input-express-num" class="form-control" />
                      <span class="input-group-btn">
                      <button onclick="checkExpressNum()" type="button" id="button-express-num" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary">Apply</button>
                      </span></div>
      <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-express-info"><?php echo 'Express Info'; ?></label>
                  <div class="col-sm-10">
                    <div id="input-express-info" class="input-group">
                  	</div>
                  </div>
     </div>
     <div class="text-right">
              <button id="button-express" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?php echo $button_express_add; ?></button>
            </div>
      </div>
  ...
  <script type="text/javascript" src="/admin/view/javascript/17track.js"></script>
  <script type="text/javascript"><!--
//Express Num yul
  function checkExpressNum() {
  var a = $("#input-express-num").val();
      if(a==""){
          alert("Enter Your Express Num."); 
          return;
      }
      YQV5.trackSingle({
          YQ_ContainerId:"input-express-info",
          YQ_Height:400,
          YQ_Fc:"0",
          YQ_Lang:"en",
          YQ_Num:a
      });
      $("#input-express-info").delegate(".result-bar .ex-asd", function(){$(".result-bar .ex-asd").empty();});
  }
...
$('#button-history').on('click', function() {    
	$.ajax({
		url: '<?php echo $store_url; ?>index.php?route=api/order/history&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
...
$('#button-express').on('click', function() {
	...
	$.ajax({
		url: 'index.php?route=sale/order/addExpressNum&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		data: 'express_num=' + encodeURIComponent($('#input-express-num').val()),
		beforeSend: function() {
		...
		success: function(json) {
			$('.alert').remove();
			if (json['error']) {
				$('#express').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
			}

			if (json['success']) {
				$('#express').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
