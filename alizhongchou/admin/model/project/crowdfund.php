...
class ModelProjectCrowdFund extends Model {
	...
	public function add($data){
	  ...
	  $this->db->query("UPDATE cf_crowdfunding set recommend=".$data['recommend']." where funding_id='".$data['funding_id']."'");	  
	  $arr=$this->db->query("select a.name,b.firstname,b.lastname from cf_crowdfunding as a left join cf_customer as b on a.customer_id=b.customer_id where a.funding_id=".$data['funding_id']);
    ...
    public function info($funding_id){
	    ...
	    $que = $this->db->query("SELECT recommend from cf_crowdfunding WHERE funding_id=".$funding_id);
		$arr=array();
		foreach($query->rows as $k=>$v){
		   foreach($que->rows as $k=>$vv){
		      $arr[$k]['funding_id']=$v['funding_id'];
			  $arr[$k]['status']=$v['status'];
			  $arr[$k]['notification']=$v['notification'];
			  $arr[$k]['content']=$v['content'];
			  $arr[$k]['recommend']=$vv['recommend'];
		   }
		}
		return $arr;
	}
