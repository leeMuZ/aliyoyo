...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		$data['entry_user_background_default'] = $this->language->get('entry_user_background_default');
		...
		if($data['config_background_image'] == ""){
			$data['config_background_image']=array();
		}
		
		foreach($data['config_background_image'] as $k => $v){
			if ($v && is_file(DIR_IMAGE . $v)) {
				$data['backgroundImage'][] = $this->model_tool_image->resize($v, 100, 100);
			} else {
