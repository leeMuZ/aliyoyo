...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		$data['livehelp'] = $this->load->controller('livehelp/livehelp_widget');	
	$data['description'] = $this->document->getDescription();
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			$article_total = $this->model_blog_article->evaluationNotify();
			$data['article_total'] = $article_total;
			$data['article_approval'] = $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'] . '&cat=34&filter_status=0', true);
			$data['alerts'] = $customer_total + $product_total + $review_total + $return_total + $affiliate_total+$article_total;
			// Online Stores
			$data['stores'] = array();
