...
class ControllerSaleOrder extends Controller {
	...
	//add by yagami at 20161019
	public function addExpressNum() {
	   $this->load->model('sale/order');
	   $json = array();
	   if ((utf8_strlen(trim($this->request->post['express_num'])) < 0) || (utf8_strlen(trim($this->request->post['express_num'])) > 64)) {
	        
	       $json['error'] = "expressNum must be between 1 and 64 characters! ";
	   }else{
	       $data = array(
	           'express_num' => $this->request->post['express_num'],
	           'order_id' => $this->request->get['order_id']
	       );
	       $express_num = $this->model_sale_order->createOrderExpressNum($data);
	        
	       $json['success'] = "You have modified express_num! ";
	   }
	...
	protected function getList() {
		...
		$data['sort_nickname'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.nickname' . $url, true);
		$data['sort_email'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.email' . $url, true);
...
*/
	...
	public function getForm() {
		...
		$data['entry_express_num'] = 'Express Num';
		...
		if (!empty($order_info)) {
			...
			$data['express_num'] = $order_info['express_num'];
	...
	public function info() {
		...
		//$order_review = $this->model_sale_order->getreview($order_id);
		$data['order_statuses'] = $this->model_sale_order->getOrderstatus();
		if ($order_info) {
			$this->load->language('sale/order');
			...
			$data['text_payment_telephone'] = $this->language->get('text_payment_telephone');
			...
			// Shipping Address
			$order_infor = $this->model_sale_order->getOrdertext($order_id);
			if ($order_infor['shipping_address_format']) {
				$format = $order_infor['shipping_address_format'];
			} else {
