...
class ModelProjectRewards extends Model {
    ...
    public function getReward($rewards_id) {
        $query = $this->db->query("SELECT
             funding_id,
		     rewards_id,
		     crowdfunding_name,
             ...
             DATE_FORMAT(limit_time_end,'%Y-%m-%d') as limit_time_end,
		     stock
		     FROM " . DB_PREFIX . "crowdfunding_rewards WHERE rewards_id = '" . $rewards_id . "'");
        return $query->row;
    }
    ...
    public function getRewardBackerByOrderId($order_id){
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "rewards_backers WHERE order_id = '" . $order_id . "'");
        return $query->row;
    }
}