...
class ControllerPaymentPPStandard extends Controller {
	public function index() {
		...
		if ($order_info) {
		    if($order_info['rewards_id']){
		        $data['rewards'] = $this->model_project_rewards->getReward($order_info['rewards_id']);
		    }else{
		        $data['funding'] = $this->model_project_crowdfund->getCrowdFunding($order_info['funding_id']);
		    }
		    $data['coupon'] = $this->model_total_coupon->getCouponById($order_info['mycoupon_id']);
			...
			if(!empty($data['rewards'])){
			    $funding_id = $data['rewards']['funding_id'];
			}else{
			    $funding_id = '';
			}
			$data['cancel_return'] = $this->url->link('crowdfunding/campaign', 'funding_id='.$funding_id, true);
