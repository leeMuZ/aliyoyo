...
class ControllerPaymentCreditCard extends Controller
{
    ...
    public function tocreditcard()
    {
       ...
       /* foreach ($array as $v) {
        ...
        $data['action'] = "https://pay.asiabill.com/Interface";//$data['my_message']['creditcard_gateway'];//提交的网关
        ...
        $phone = empty($order_info['shipping_telephone']) ? $this->model_checkout_order->checkYouTelephone($this->session->data['customer_id']) : $order_info['shipping_telephone'];
    ...
    public function callback()
    {
        ...
        if (isset($this->request->post['orderNo']) && !(empty($this->request->post['orderNo']))) {
            ...
            $data['text_success_url'] = $data['base'] . 'index.php?route=checkout/success';
            $data['text_failure_url'] = $data['base'] . 'index.php?route=crowdfunding/back_project&funding_id=' . $this->session->data['funding_id'];
