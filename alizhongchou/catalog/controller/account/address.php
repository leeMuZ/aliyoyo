...
class ControllerAccountAddress extends Controller {
	...
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/edit#address', '', true);
			echo 1;return;
	...
	protected function getList() {
		...
		$addr_total = $this->model_account_address->getTotalAddresses();
		$results = $this->model_account_address->getAddresses(($page - 1) * 10, 10);
		if($this->customer->getAddressId() && ($page==1|| $page=="{page}" )){
		...
		foreach ($results as $result) {
			if ($result['address_format']) {
				...
				//$format = 'name: '."&nbsp&nbsp".'{firstname} {lastname}' . "\n".'telephone: '."&nbsp&nbsp".'{telephone}'."\n" . 'company: '."&nbsp&nbsp".'{company}' . "\n" . 'Street_1: '."&nbsp&nbsp".'{address_1}' . "\n" .
				// 'Street_2: '."&nbsp&nbsp".'{address_2}' . "\n" . 'city&postcode: '."&nbsp&nbsp".'{city} {postcode}' . "\n" . 'zone: '."&nbsp&nbsp".'{zone}' . "\n" .'country: '."&nbsp&nbsp".'{country}';
				$format = '<p class="name"><span>name:</span><span>{firstname} {lastname}</span></p>
							<p class="telephone"><span>telephone:</span><span>{telephone}</span></p>
							<p class="company"><span>company:</span><span>{company}</span></p>
							<p class="Street_1"><span>Street_1:</span><span>{address_1}</span></p>
							<p class="Street_2"><span>Street_2:</span><span>{address_2}</span></p>
							<p class="citypostcode"><span>city&postcode:</span><span>{city} {postcode}</span></p>
							<p class="zone"><span>zone:</span><span>{zone}</span></p>
							<p class="country"><span>country:</span><span>{country}</span></p>
							';
}
		...
		//add by nana 2016.11.07
	$addrs=array();
		if($this->customer->getAddressId()){
			$defaultaddr=$this->customer->getAddressId();
			$i=0;
			foreach ($data['addresses'] as $v){
				if($defaultaddr == $v['address_id']){
					if($page!=1 && $page!="{page}"){
						continue;
					}
					$addrs[0]=$v;
		...
		$pagination->total = $addr_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('account/address', 'page={page}', true);

		$data['pagination'] = $pagination->render();
		if($addr_total==11 && count($data['addresses'])==11){
			$data['pagination']="";
		}
		//add by nana  2016.11.07 end
...
//modify by yagami at20160709
	protected function getForm() {
		...
		//modify by yagami at 20160709 ��Ӵ���绰������Ϣ
		...
		if(count($this->error)){
			$json["error"]="error";
			$json["data"]=$this->load->view('account/address_form', $data);
	...
	public function setDefauleAddr(){
		if(isset($this->request->post['address_id'])){
			$this->load->model('account/address');
			$this->model_account_address->setDefauleAddr($this->request->post['address_id']);
			$data['code']="success";
		}else{
			$data['code']="error";
		}
