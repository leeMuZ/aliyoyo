...
class ControllerApiOrder extends Controller {
			...
			if ($order_info) {
				...
				if (!$json) {
					...
					if ($this->cart->hasShipping()) {
						...
						if (isset($this->request->post['express_num'])) {
						    $order_data['express_num'] = $this->request->post['express_num'];
						} else {
						    $order_data['express_num'] = '';
						}
					} else {
						$order_data['shipping_firstname'] = '';
						$order_data['shipping_lastname'] = '';
	...
	public function delete() {
		...
		$json = array();
			...
			$this->load->model('account/selforder');
			...
			if (isset($this->request->get['order_id'])) {
				...
				$this->model_account_selforder->deleteOrder($order_id);
	...
	public function history() {
		...
		$json = array();
			...
			if (isset($this->request->get['order_id'])) {
				...
				$this->model_account_selforder->addOrderHistory($order_id, $this->request->post['order_status_id'], $this->request->post['comment'], $this->request->post['notify'], $this->request->post['override']);
