...
class ControllerModuleBlog extends Controller {
    ...
    private function addBasicLang($array = array())
    {
        ...
        $data['link_blog_dashboard'] = $this->url->link('module/blog', 'token=' . $this->session->data['token'], true);
        $data['link_add_edit_categories'] = $this->url->link('module/blog/category_list', 'token=' . $this->session->data['token'], true);
        $data['link_add_edit_articles'] = $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'], true);
        $data['link_add_edit_customer'] = $this->url->link('module/blog/article_list', 'cat=34&token=' . $this->session->data['token'], true);
        $data['link_add_edit_authors'] = $this->url->link('module/blog/author_list', 'token=' . $this->session->data['token'], true);
        $data['link_add_edit_comments'] = $this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'], true);
        $data['link_general_settings'] = $this->url->link('module/blog/settings', 'token=' . $this->session->data['token'], true);
        
        $data['link_blog_category'] = $this->url->link('module/blog_category', 'token=' . $this->session->data['token'], true);
        $data['link_blog_search'] = $this->url->link('module/blog_search', 'token=' . $this->session->data['token'], true);
        $data['link_blog_latest_post'] = $this->url->link('module/blog_latest', 'token=' . $this->session->data['token'], true);
        $data['link_blog_popular_post'] = $this->url->link('module/blog_popular', 'token=' . $this->session->data['token'], true);
        $data['link_blog_product_related_post'] = $this->url->link('module/blog_related_post', 'token=' . $this->session->data['token'], true);
        $data['link_blog_popular_tags'] = $this->url->link('module/blog_tags', 'token=' . $this->session->data['token'], true);
	...
	public function index() {   
        ...
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/blog', $data));
	}
    ...
    public function article_list()
    {
		...
		$this->load->language('module/blog/article');
	...
	public function article_add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateArticleForm()) {
			/* add by yss @2016-11-30 modify evalution add BUG*/
			$this->request->post['article_rating']=array(
					array("option"=>"Appearance","value"=>10),
					array("option"=>"Design","value"=>10),
					array("option"=>"Endurance","value"=>10),
					array("option"=>"Function","value"=>10),
					array("option"=>"Material","value"=>10)
			);
			/* END */
...
//Yul
			...
			$this->response->redirect($this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'] . $url, true));
		}
		$this->getArticleForm();
	}
...
/* Add by Yul
	 * @20160505
	 * For Blog Upload Image
	 */
    private function save_blog_image($str = '', $s = '', $e = ''){
        $r = explode($s, $str);
        array_splice($r, 0, 1);
        if (!empty($r)){
            $b = array();
            foreach($r as $k=>$v){
                $a[$k] = explode($e, $v);
                array_splice($a[$k],1,1);                
                $b[] = $a[$k][0];
            }
            $b = array_unique($b);
            foreach ($b as $v){
                $img_name = '/image/catalog/blog/' . time() .rand(1000,9999);                
                if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $v, $result)){
                    $img_name = $img_name . '.' . $result[2];
                    if (file_put_contents($_SERVER['DOCUMENT_ROOT'] . str_replace('\\', '/', $img_name), base64_decode(str_replace($result[1], '', $v)))){
                        $str = str_replace($v, 'http://' . $_SERVER['HTTP_HOST'] . $img_name, $str);
                    }                
                }
            }
            return $str;
	...
	public function article_edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateArticleForm()) {
		    ...
		    //$post_arr["article_description"][1]["content"] = urlencode($this->save_blog_image($_POST["article_description"][1]["content"], 'src="', '" style'));
		    $this->request->post["article_description"][1]["content"] = $this->save_blog_image($_POST["article_description"][1]["content"], 'src="', '" style');
		     
            $this->model_blog_article->editArticle($this->request->get['article_id'], $this->request->post);
		    //$this->model_blog_article->editArticle($this->request->get['article_id'], $post_arr);
		    //END
	...
	protected function getArticlesList() {
		...
		//Yul
		if (isset($this->request->get['cat'])) {
		    $filter_cat = (int)$this->request->get['cat'];
		} else {
		    $filter_cat = null;
		}
		// add by yagami 20161020
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'], true)
		);
		
		
		$data['add'] = $this->url->link('module/blog/article_add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('module/blog/article_delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['articles'][] = array(
				...
				'author'  => '<a target="_blank" href="'.$this->url->link('module/blog/author_list', 'token=' . $this->session->data['token'] . '&author_id=' . $result['author_id'] . $url, true).'">'.$result['author'] .'</a>',
                'comments'    => $this->model_blog_comment->getTotalArticleComments($result['article_id']),
				'edit'        => $this->url->link('module/blog/article_edit', 'token=' . $this->session->data['token'] . '&article_id=' . $result['article_id'] . $url, true),
				'delete'      => $this->url->link('module/blog/article_delete', 'token=' . $this->session->data['token'] . '&article_id=' . $result['article_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'] . '&sort=title' . $url, true);
		$data['sort_date_published'] = $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'] . '&sort=date_published' . $url, true);
		$data['sort_author'] = $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'] . '&sort=author' . $url, true);
		...
		$pagination->url = $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/article_list', $data));
	}
...
//Yul
		if (isset($this->request->get['cat'])) {
		    $filter_cat = "&cat=" . (int)$this->request->get['cat'];
		    $url .= $filter_cat;
			$data['cat']=(int)$this->request->get['cat'];
		}
		
		if (!isset($this->request->get['article_id'])) {
			$data['action'] = $this->url->link('module/blog/article_add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('module/blog/article_edit', 'token=' . $this->session->data['token'] . '&article_id=' . $this->request->get['article_id'] . $url, true);
		}
        
		$data['cancel'] = $this->url->link('module/blog/article_list', 'token=' . $this->session->data['token'] . $url, true);
        ...
        $data['is_customer_evaluation'] = 1;
        if(empty($article_info['author_id'])){
            $data['authors'][]=array(
                'customer_id' => 0,
                'nickname' => ' '
                );
            $data['is_customer_evaluation'] = 0;
        }else{
         
            foreach ($data['authors'] as $author){
                if($author['customer_id']==$article_info['author_id']){
                    $data['is_customer_evaluation'] = 0;
                }
            }
       ...
       /*  var_dump( $data['authors']);
		...
		if (isset($this->request->post['fancy'])) {
		    $data['fancy'] = $this->request->post['fancy'];
		} elseif (!empty($article_info)) {
		    $data['fancy'] = $article_info['fancy'];
		} else {
		    $data['fancy'] = '';
		}
		...
		if(count($data['product_relateds']) == 1){
		    $this->load->model('catalog/category');
		    if (isset($this->request->post['article_rating'])) {
		        $data['article_rating'] = $this->request->post['article_rating'];
		    }elseif(isset($this->request->get['article_id'])) {
		        if(isset($this->request->get['cat'])) {
		            $data['is_evaluation'] = 1;
		        }
		        $data['article_rating'] = $this->model_catalog_product->getProductRatingOption($data['product_relateds'][0]['product_id'], $this->request->get['article_id'], 'professional');
			if(!$data['article_rating']){
    		        $rating_category = $this->model_catalog_product->getProductRatingCategory($data['product_relateds'][0]['product_id']);
    		        $rating_des = $this->model_catalog_category->getRatingCategories($rating_category);
    		        foreach ($rating_des as $value){
                        $data['article_rating'][$value['rating_cat_id']]['rating_description'] = $value['rating_cat_name'];  
                        $data['article_rating'][$value['rating_cat_id']]['rating_value'] = 10;    		            
    		        }           
		        }
		    }else{
		        $rating_cat = $this->model_catalog_product->getProductRatingOption($data['product_relateds'][0]['product_id'], 'model');
			
		        $data['article_rating'] = $this->model_catalog_category->getRatingCategories($rating_cat);
		    }
		}
		//END
		...
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/article_form', $data));
	}
    ...
    {
		$this->load->language('module/blog/category');
	...
	public function category_add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateCategoryForm()) {
			...
			$this->response->redirect($this->url->link('module/blog/category_list', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getCategoriesList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/blog/category_list', 'token=' . $this->session->data['token'], true)
		);

		$data['add'] = $this->url->link('module/blog/category_add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('module/blog/category_delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'edit'        => $this->url->link('module/blog/category_edit', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true),
				'delete'      => $this->url->link('module/blog/category_delete', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('module/blog/category_list', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('module/blog/category_list', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('module/blog/category_list', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/category_list', $data));
	}
	...
	protected function getCategoryForm() {
		...
		if (!isset($this->request->get['category_id'])) {
			$data['action'] = $this->url->link('module/blog/category_add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('module/blog/category_edit', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('module/blog/category_list', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/category_form', $data));
	}
    ...
    public function comment_list() {
		$this->load->language('module/blog/comment');
	...
	public function comment_add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateCommentForm()) {
			...
			$this->response->redirect($this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getCommentList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'], true)
		);
        
		$data['add'] = $this->url->link('module/blog/comment_add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('module/blog/comment_delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['comments'][] = array(
				...
				'date_added'      => $result['date_added'],
                		...
                		'article_href'     => $this->url->link('module/blog/article_edit', 'token=' . $this->session->data['token'] . '&article_id=' . $result['article_id'], true),
				'edit'            => $this->url->link('module/blog/comment_edit', 'token=' . $this->session->data['token'] . '&comment_id=' . $result['comment_id'] . $url, true)
			);}
		...
		$data['sort_name'] = $this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_email'] = $this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'] . '&sort=email' . $url, true);
		$data['sort_date_added'] = $this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, true);
		$data['sort_status'] = $this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('module/blog/comment_list', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/comment_list', $data));
	}
	...
	protected function getCommentForm() {
		...
		if (!isset($this->request->get['comment_id'])) {
			$data['action'] = $this->url->link('module/blog/comment_add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('module/blog/comment_edit', 'token=' . $this->session->data['token'] . '&comment_id=' . $this->request->get['comment_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('module/blog/comment', 'token=' . $this->session->data['token'] . $url, true);
        ...
        $data['article_href'] = $this->url->link('module/blog/article_edit', 'token=' . $this->session->data['token'] . '&article_id=' . $comment_info['article_id'], true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/comment_form', $data));
	}
    ...
    public function author_list() {
		$this->load->language('module/blog/author');
	...
	public function author_add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateAuthorForm()) {
			...
			$this->response->redirect($this->url->link('module/blog/author_list', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getAuthorList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/blog/author_list', 'token=' . $this->session->data['token'], true)
		);
		//modify by yagami at 20161022
		//$data['add'] = $this->url->link('module/blog/author_add', 'token=' . $this->session->data['token'] . $url, true);
		$data['add'] = $this->url->link('customer/customer', 'token=' . $this->session->data['token'] . $url."&filter_customer_group_id=4", true);
		$data['delete'] = $this->url->link('module/blog/author_delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['authors'][] = array(
				'author_id' => $result['customer_id'],
				'name'            => $result['nickname'],
				'date_added'      => $result['date_added'],
				/*don't delete ! yagami at 20161014  */
				//'edit'            => $this->url->link('module/blog/author_edit', 'token=' . $this->session->data['token'] . '&author_id=' . $result['author_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('module/blog/author_list', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_date_added'] = $this->url->link('module/blog/author_list', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, true);
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('module/blog/author_list', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/author_list', $data));
	}
	...
	protected function getAuthorForm() {
		...
		if (!isset($this->request->get['author_id'])) {
			$data['action'] = $this->url->link('module/blog/author_add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('module/blog/author_edit', 'token=' . $this->session->data['token'] . '&author_id=' . $this->request->get['author_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('module/blog/author', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['footer'] = $this->load->controller('common/footer');
        $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/author_form', $data));
	}
    ...
    // SETTIGNGS
		...
		$this->load->language('module/blog/settings');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateSettingsForm()) {
			...
			$this->response->redirect($this->url->link('module/blog/settings', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getSettingsForm() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/blog/settings', 'token=' . $this->session->data['token'], true)
		);

        $data['action'] = $this->url->link('module/blog/settings', 'token=' . $this->session->data['token'] . $url, true);
		$data['cancel'] = $this->url->link('module/blog', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['footer'] = $this->load->controller('common/footer');
          $data['handy_box'] = $this->load->view('module/blog/partial/header', $data);
		$this->response->setOutput($this->load->view('module/blog/settings_form', $data));
	}
