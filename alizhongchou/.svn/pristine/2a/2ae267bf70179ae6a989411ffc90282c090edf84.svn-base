...
class ModelToolImage extends Model {
	...
	public function upload($data = array()) {
		...
		if (!isset($data['directory'])) {
			$json['error'] = $this->language->get('empty_directory');
		}
		if (empty($data['files'])) {
					...
					/*  if($this->config->get('amazon_s3_status')){
					    ...
					    if(is_dir(DIR_IMAGE . $data['directory'])){
						 move_uploaded_file($data['files']['tmp_name'][$i], DIR_IMAGE . $url);
					        ...
					        $path = '';
					        $directories = explode('/', dirname(str_replace('../', '', $url)));
					        foreach ($directories as $directory) {
					            $path = $path . '/' . $directory;
						...
						$json['images'][$i]['url'] = $url;
					$json['images'][$i]['success'] = $this->language->get('text_uploaded');
				}else{
					break;
				}
			}
