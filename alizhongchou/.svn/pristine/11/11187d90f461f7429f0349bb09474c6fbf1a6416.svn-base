              ...
              <thead>
                <tr>
                  ...
                  <td class="text-left"><?php if ($sort == 'm.name') { ?>
                    ...
                    <?php } ?></td>
                  <td class="text-left"><?php if ($sort == 'm.category') { ?>
                    ...
                    <?php } ?></td>
                  <td class="text-right"><?php if ($sort == 'm.price') { ?>
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-center"><?php if (in_array($model['model_id'], $selected)) { ?>
                    ...
                    <?php } ?></td>
                  <td class="text-center"><?php if ($model['image']) { ?>
                    <img src="<?php echo $model['image']; ?>" alt="<?php echo $model['name']; ?>" class="img-thumbnail" />
                    <?php } else { ?>
                    <span class="img-thumbnail list"><i class="fa fa-camera fa-2x"></i></span>
                    <?php } ?></td>
                  <td class="text-left"><?php echo $model['name']; ?></td>
                  <td class="text-left"><?php echo $model['category_name']; ?></td>
                  <td class="text-left"><?php echo $model['price']; ?></td>
                  <td class="text-left"><?php echo $model['status']; ?></td>
...
$('#button-filter').on('click', function() {
	var url = 'index.php?route=catalog/model&token=<?php echo $token; ?>';
...
$('input[name=\'filter_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/model/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
...
$('input[name=\'filter_category\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/model/autocompleteCategory&token=<?php echo $token; ?>&filter_category=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
						label: item['category_name'],
						value: item['model_category_id']
					}
				}));
			}
		});
