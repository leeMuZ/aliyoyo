...
class ModelTotalCoupon extends Model {
	public function getCoupon($code) {
		$status = 1;
		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE coupon_code = '" . $this->db->escape($code) . "' AND (UNIX_TIMESTAMP(NOW()) BETWEEN UNIX_TIMESTAMP(fromtime) AND UNIX_TIMESTAMP(totime))");
        if ($coupon_query->row)
        {
            return array(
                ...
                'balance'       => $coupon_query->row['balance'],
                'minbalance'    => $coupon_query->row['minbalance'],
                'date_start'    => $coupon_query->row['fromtime'],
                'date_end'      => $coupon_query->row['totime'],
                'uses_total'    => $coupon_query->row['nums'],
                'uses_customer' => $coupon_query->row['numcount'],
                'date_added'    => $coupon_query->row['addtime']
            );
        }else{
            return false;
        }
	...
	public function getCouponById($id) {
	    $coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE coupon_id = '" . $this->db->escape($id) . "'");
	...
	public function getCouponForName($name){
		$res=$this->db->query("SELECT code FROM `" . DB_PREFIX . "coupon` WHERE name = '" . $this->db->escape($name) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1'");
		return $res->row['code'];
	}
}
