...
class ControllerCatalogProduct extends Controller {
    ...
    protected function getForm() {
        ...
        //add by yss @2016-12-22 report and coupon
        if (isset($this->request->post['report'])) {
            $data['report'] = $this->request->post['report'];
        } elseif (!empty($product_info)) {
            $data['report'] = $product_info['report'];
        } else {
            $data['report'] = '';
        }
        ...
        if (isset($this->request->post['coupon'])) {
            ...
            $data['coupon'] = $product_info['coupon'];
        } else {
            $data['coupon'] = '';
        }

        //add by yss @2016-12-22 video begin
        ...
        //add by yss @2016-12-22 video end

        // add by yss @2017-01-17 product type and link
        ...
        if (isset($this->request->post['recommend_href'])) {
            $data['recommend_href'] = $this->request->post['recommend_href'];
        } elseif (!empty($product_info)) {
            $data['recommend_href'] = $product_info['recommend_href'];
        } else {
            $data['recommend_href'] = '';
        }
...
//Yul
        $data['rating_cats'] = $this->model_catalog_category->getRatingCategories();
        if (isset($this->request->post['rating_cat_id'])) {
            $data['rating_cat_id'] = $this->request->post['rating_cat_id'];
        } elseif (isset($this->request->get['product_id'])) {
            $data['rating_cat_id'] = $this->model_catalog_product->getProductRatingCategory($this->request->get['product_id']);
        } else {
            $data['rating_cat_id'] = array();
        }
//END
    ...
    public function autoFuture()
    {
        ...
        if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']))
        {
            ...
            $results = $this->model_catalog_product->getFutures($filter_data);
            foreach ($results as $result)
            {
                $json[] = array(
                    ...
                    'name'       => strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')),
                    'image'      => $result['image'],
                    'show_image' => $this->image_ajax($result['image'])
                );
    ...
    public function autoCategory()
    {
        ...
        if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']))
        {
            ...
            $results = $this->model_catalog_product->getCategories($filter_data);
            foreach ($results as $result)
            {
                $json[] = array(
