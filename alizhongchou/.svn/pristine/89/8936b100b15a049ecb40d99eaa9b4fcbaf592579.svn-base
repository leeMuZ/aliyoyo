...
class ModelReportProduct extends Model {
	...
	public function getSearchReport($data = array()) {
	    $sql="SELECT * FROM `" . DB_PREFIX . "search_report`";
	    if (isset($data['start']) || isset($data['limit'])) {
	        if ($data['start'] < 0) {
	            $data['start'] = 0;
	        }
	        ...
	        $sql .= "order by search_id desc LIMIT " . (int)$data['start'] . "," . (int)$data['limit'];
	    }
	    $query = $this->db->query($sql);
	    return $query->rows;
	}
	public function getTotalSearch() {
	    $sql="SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "search_report`";
	    $query = $this->db->query($sql);
	    return $query->row['total'];
	}
