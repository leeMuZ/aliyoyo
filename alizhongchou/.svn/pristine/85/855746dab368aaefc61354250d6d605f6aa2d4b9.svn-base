        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-setting" class="form-horizontal">
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane" id="tab-image">
              ...
              <div class="form-group" id="userBackgroundDefaultImage">
                <label class="col-sm-2 control-label" for="input-icon"><?php echo $entry_user_background_default; ?></label>
                <?php foreach($config_background_image as $k => $v){?>
                <div style="float:left;margin-right:15px;"><a href="" id="thumb-icon_<?php echo $k+1;?>" data-toggle="image" class="img-thumbnail"><img src="<?php echo $backgroundImage[$k]; ?>" alt="" title="" data-placeholder="<?php echo $placeholder; ?>" /></a>
                  <input type="hidden" name="config_background_image[]" value="<?php echo $v; ?>" id="config_background_image_<?php echo $k+1;?>" />
                  <input type="button" name="delUserBackgroundDefaultImage" value="del"/>
                </div>
                <?php }?>
              </div>
              <input type="button" value="add image" data-word="8" name="addUserBackgroundDefaultImage"/>
            <script>
            $("[name='addUserBackgroundDefaultImage']").on("click",function(){
                var num=$(this).attr("data-word");
                num++;
                var html="";
                html+='<div style="float:left;margin-right:15px;"><a href="" id="thumb-icon_'+num+'" data-toggle="image" class="img-thumbnail"><img src="/image/no_image.jpg" width="100px" height="100px" data-placeholder="/image/no_image.jpg"/></a>';
                html+='<input type="hidden" name="config_background_image[]" value="" id="config_background_image_'+num+'" /><input type="button" name="delUserBackgroundDefaultImage" value="del"/></div>';
				$(this).attr("data-word",num);
                $("#userBackgroundDefaultImage").append(html);
            });
            $(document).delegate("[name='delUserBackgroundDefaultImage']","click",function(){
                $(this).parent().remove();
            });
            </script>
            </div>
