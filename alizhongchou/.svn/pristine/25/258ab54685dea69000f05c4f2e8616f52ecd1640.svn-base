...
class ModelCheckOutOrder extends Model
{
    ...
    public function addOrderHistory($order_id, $order_status_id, $comment= '', $notify = false, $override = false)
    {
		...
		if ($order_info) {
            ...
            $mail->Username = "customerservice@aliyoyo.com";  //你的邮箱
            $mail->Password = html_entity_decode("Aliyoyo168", ENT_QUOTES, 'UTF-8');  //你的密码
            $mail->Subject = $subject;//邮件标题

            $mail->From = "customerservice@aliyoyo.com";  //发件人地址（也就是你的邮箱）
            $mail->FromName = "Aliyoyo";  //发件人姓名

            $address = "customerservice@aliyoyo.com";//收件人email
            $mail->AddAddress($address);//添加收件人（地址，昵称）

            $mail->IsHTML(true); //支持html格式内容
            $mail->Body = $this->load->view('mail/order', $data);
    ...
    {
        $query = $this->db->query("SELECT telephone FROM ".DB_PREFIX ."customer WHERE customer_id=".$customer_id);
