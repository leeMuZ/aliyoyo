...
class ControllerAccountPassword extends Controller {
	...
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/edit#password', '', true);
	...
	protected function validate() {
		//add by nana 2016.11.5 begin
		if (!$this->customer->login($this->customer->getEmail(), $this->request->post['current'])) {
			$this->error['current'] = $this->language->get('error_current');
		}
		//add by nana 2016.11.5end
