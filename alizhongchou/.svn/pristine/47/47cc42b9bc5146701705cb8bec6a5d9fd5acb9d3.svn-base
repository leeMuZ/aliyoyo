		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
		   ...
		   if(isset($this->request->post['action']) && $this->request->post['action']=='import') {
			...
			//$save['xshippingpro']=base64_encode(serialize($this->request->post['xshippingpro']));
			    $save_set = array();
    			foreach ($this->request->post['xshippingpro'] as $key => $value){
    			    //$save_set['xshippingproset_' . $key] = json_encode($value);
    			    $save_set['xshippingproset_' . $key] = base64_encode(serialize($value));
			        $this->model_setting_setting->editSetting('xshippingproset', $save_set);
    			}
			}
...
/* 		$xshippingpro=$this->config->get('xshippingpro');
		if ($xshippingpro) {
            $xshippingpro=json_decode($xshippingpro,true);
		} */
		
		//$xshippingproset=$this->config->get('xshippingproset');
		$xshippingpro = array();
		$xshippingproset = array();
		$xshippingproset = $this->model_setting_setting->getSettingValueByCode("xshippingproset");
		//var_dump($xshippingproset);exit;
		if ($xshippingproset) {
		    //$xshippingpro=unserialize(base64_decode($xshippingpro)); 
		    foreach ($xshippingproset as $key => $value){
		        $xshippingpro[str_replace('xshippingproset_', '', $value['key'])] = unserialize(base64_decode($value['value']));
		        //var_dump($value);exit;
		        //foreach (json_decode($value['value'],true) as $k => $v){
		            //$xshippingpro[$k] = json_decode($v,true);
		            //$xshippingpro[$k] = unserialize(base64_decode($v));
		        //}
    		        //$xshippingpro = array_merge_recursive($xshippingpro, get_object_vars(json_decode($value['value'])));
    		        //$xshippingpro[] = json_decode($value['value'],true);
    		        //var_dump($xshippingpro);exit;
		    }
		}
		//var_dump($xshippingpro);exit;
      ...
      public function quick_save(){
        ...
        $this->load->model('setting/setting');
         ...
         if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			 ...
			 if(isset($this->request->post['xshippingpro'])){
			     //var_dump($this->request->post['xshippingpro']);exit;
			 //$save['xshippingpro']=base64_encode(serialize($this->request->post['xshippingpro']));
			 $save_set = array();
    			foreach ($this->request->post['xshippingpro'] as $key => $value){
    			    $save_set['xshippingproset_' . $key][$key] = json_encode($value);
			        $this->model_setting_setting->editSetting('xshippingproset', $save);
    			}
			 }
			 $this->model_setting_setting->editSetting('xshippingpro', $save);
			 $json['success']=1;
